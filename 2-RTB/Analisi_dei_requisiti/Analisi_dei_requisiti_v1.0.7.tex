    ...
    \begin{center}
        ...
        \begin{center}
            \begin{tabular}{|c|c|c|c|}
                ...
                \hline
                1.0.7 & 15/12/2025 & Da verificare &       
                \begin{tabular}[c]{@{}l@{}}
...
\begin{center}
    \begin{tabularx}{\textwidth}{|c|c|c|c|c|X|}
        ...
        \hline
        1.0.7 & 15/12/2025 & F. Venzo & - & - & Aggiunti diagrammi UML Use Cases UC1, UC1.4, UC1.5, UC2, UC2.8, UC2.9  \\
        \hline
        ...
        \hline
        1.0.0 & 5/12/2025 & F. Venzo & L. Pellizzon & F. Pasqual & Approvazione del documento per l'invio alla proponente.\\
        \hline
...
\addScenarioPrincipale{L'\admin crea uno username per l'\os;}
\addScenarioPrincipale{L'\admin crea una password temporanea per l'\os;}
...
\addAttore{\admin.}
\addPreCondizione{L'\admin sta creando un nuovo utente \os;}
\addPostCondizione{Il Sistema conosce lo username delL'\os;}
\addScenarioPrincipale{L'\admin sceglie uno username per l'\os}
...
\addAttore{\admin.}
\addPreCondizione{L'\admin sta creando un nuovo utente \os;}
\addPostCondizione{Il Sistema conosce la password temporanea per l'\os;}
...
\addPreCondizione{Il Sistema è attivo;}
\addPostCondizione{L'utente è autenticato nel Sistema;}
\addPostCondizione{L'\userna è riconosciuto come \os.}

\addScenarioPrincipale{L'\userna si autentica con le credenziali fornite dall'\admin; (vedi Creazione nuovo utente \os \rif{1.4})}
\addScenarioPrincipale{Il Sistema obbliga l'\os ad aggiornare la Password temporanea;}

\addInclusione{Autenticazione \rif{1}}
...
\addPostCondizione{L'\os ha aggiornato la propria Password.}

\addScenarioPrincipale{L'\os inserisce la Password attuale;}
\addScenarioPrincipale{L'\os inserisce una nuova Password;}
\addScenarioPrincipale{L'\os conferma la nuova Password reinserendola.}

\addScenarioAlternativo{L'\os inserisce la Password attuale come nuova Password;}
\addScenarioAlternativo{L'\os inserisce una nuova Password non valida;}
\addScenarioAlternativo{L'\os non conferma correttamente la nuova Password.}
\addScenarioAlternativo{L'\os inserisce la propria Password attuale errata;}


\addEstensione{Errore Password attuale ripetuta \rif{1.5.2}}
\addEstensione{Errore nuova Password non valida \rif{1.5.3}}
\addEstensione{Errore conferma nuova Password \rif{1.5.4}}
\addEstensione{Errore Password attuale errata \rif{1.5.5}}
\makeUC{1.5.1}{Aggiornamento Password}

\addAttore{\os.}
\addPreCondizione{L'\os sta aggiornando la propria Password;}
\addPostCondizione{La password dell'\os non è stata aggiornata.}
\addScenarioPrincipale{Il Sistema segnala l'errore di Password attuale errata.}
\addScenarioPrincipale{Il sistema ripropone all'\os la schermata di aggiornamento Password.}
\addTrigger{L'\os inserisce una Password attuale errata.}
\addInclusione{Aggiornamento Password \rif{1.5.1}}
\makeUC{1.5.2}{Errore Password attuale ripetuta}
...
\addInclusione{Aggiornamento Password \rif{1.5.1}}
\makeUC{1.5.3}{Errore nuova Password non valida}

\addAttore{\os.}
\addPreCondizione{L'\os sta aggiornando la propria Password;}
\addPostCondizione{La password dell'\os non è stata aggiornata.}
\addScenarioPrincipale{Il Sistema segnala l'errore di conferma della nuova Password errata.}
\addScenarioPrincipale{Il sistema ripropone all'\os la schermata di aggiornamento Password.}
\addTrigger{L'\os non conferma correttamente la nuova Password.}
\addInclusione{Aggiornamento Password \rif{1.5.1}}
\makeUC{1.5.4}{Errore conferma nuova Password}
...
\addScenarioPrincipale{Il Sistema segnala l'errore di Password attuale errata.}
\addScenarioPrincipale{Il sistema ripropone all'\os la schermata di aggiornamento Password.}
\addTrigger{L'\os inserisce la propria Password attuale errata.}
\makeUC{1.5.5}{Errore Password attuale errata}
...
\addScenarioPrincipale{L'utente visualizza i Moduli opzionali presenti tra:
\begin{itemize}
...
L'utente visualizza il seguente Modulo fisso (non rimovibile):
\begin{itemize}
...
\begin{itemize}
    \item Numero di allarmi risolti;
    \item Numero di allarmi risolti suddivisi per tipologia;
    \item Tempo medio di risoluzione allarmi;
    \item Allarmi risolti per residenza;
    \item Distribuzione Allarmi risolti per Priorità;
\end{itemize}
}
...
\begin{itemize}
    \item Temperatura media giornaliera per residenza;
    \item Variazione della temperatura nell'arco della giornata per residenza;
    \item Differenza tra temperatura impostata e temperatura reale per residenza;
    \item Differenza di temperatura tra le varie residenze.
\end{itemize}
}
...
\begin{itemize}
    \item Consumo in kWh giornaliero per residenza;
    \item Andamento in kWh nell'arco della giornata per residenza;
    \item Media consumo in kWh dell'ultima settimana per residenza.
\end{itemize}
}
...
\addInclusione{Selezione Modulo non presente \rif{2.9.1}}
\addEstensione{Selezione Modulo presente \rif{2.8.1}}
\addEstensione{Limite Moduli raggiunto \rif{2.8.2}}
...
\addScenarioPrincipale{L'utente seleziona un Modulo, presente nella Dashboard, tra:
                        \begin{itemize}
                            \item Informazioni utente;
                            \item statistiche allarmi;
                            \item analisi clima;
...
\makeUC{2.8.1}{Selezione Modulo presente nella Dashboard}
...
\addInclusione{Selezione Modulo presente \rif{2.8.1}}
...
\addScenarioPrincipale{L'\os seleziona il gruppo di impianti che si vuole analizzare (\rif{3.10});}
...
\begin{itemize}
    \item Suggerimenti per il risparmio energetico (\rif{3.1});
    \item grafico sulle anomalie di impianto (\rif{3.2});
    \item grafico sul calcolo dell’energia consumata dall’illuminazione, a partire dal tempo (\rif{3.3});
    \item grafico sul rilevamento di presenza, assenza e caduta (\rif{3.4});
    \item grafico sul rilevamento di presenza prolungata nello stesso ambiente (\rif{3.5});
    \item grafico di variazione e cambio di temperatura (\rif{3.6});
    \item grafico sugli allarmi inviati e risolti per giorno dagli operatori sanitari (\rif{3.7});
    \item grafico sulla frequenza degli allarmi rilevati nell’arco di un periodo a scelta, suddivisi per ore (\rif{3.8});
    \item grafico sulla frequenza delle cadute rilevate nell’arco di un periodo a scelta, suddivise per ore (\rif{3.9}).
\end{itemize}
Inoltre, è possibile modificare la visualizzazione dei seguenti moduli:
\begin{itemize}
    \item Grafico sul calcolo dell’energia consumata dall’illuminazione (inserimento \rif{3.3.1} e rimozione \rif{3.3.2});
    \item grafico di variazione e cambio di temperatura (inserimento \rif{3.6.1} e rimozione \rif{3.6.2});
    \item grafico sugli allarmi inviati e risolti (inserimento \rif{3.7.1} e rimozione \rif{3.7.2}).
\end{itemize}}
\addInclusione{\rif{3.10}}
...
Come definito nella tabella, il caso d'uso 3 include 10 casi d'uso: 
\begin{center}
    \includegraphics[width=\textwidth]{./img/UC3_inclusioni.pdf}
    \captionof{figure}{UC3 Relazioni di inclusione}
\end{center}
...
\addScenarioPrincipale{L'\os seleziona il periodo temporale da analizzare (ultimi 30 giorni oppure ultimi 60 giorni).}
\addScenarioAlternativo{Il Sistema non dispone di alcun dato relativo al periodo temporale scelto (\rif{7}).}
\addEstensione{\rif{7}}
...
% ======= UC Analytics: Selezione gruppo di impianti da analizzare  =======
\addAttore{\os.}
\addPreCondizione{L'\os si trova nella sezione Analytics.}
\addPostCondizione{Il gruppo di impianti selezionato è impostato come filtro attivo per la visualizzazione delle Analytics.}
\addTrigger{L’\os vuole selezionare un gruppo di impianti da analizzare.}
\addScenarioPrincipale{L'\os seleziona uno tra i seguenti gruppi di impianti:
\begin{itemize}
    \item Tutti gli impianti;
    \item un singolo reparto (\rif{3.10.1}).
\end{itemize}}
\addInclusione{\rif{3.10.1}}
\makeUC{3.10}{Selezione del gruppo di impianti da analizzare}


% ======= UC Analytics: Visualizzazione dei reparti  =======
\addAttore{\os.}
\addPreCondizione{L'\os si trova nella sezione Analytics.}
\addPostCondizione{Il Sistema mostra l'elenco dei reparti disponibili.}
\addTrigger{L'\os vuole visualizzare l'elenco dei reparti.}
\addScenarioPrincipale{Il Sistema recupera l'elenco dei reparti esistenti disponibili per la selezione.}
\makeUC{3.10.1}{Visualizzazione dei reparti}
...
\begin{itemize}
    \item Termostato (\rif{4.1});
    \item tapparelle (\rif{4.2});
    \item sensori \textit{Ultra Wide Band} di caduta (\rif{4.3});
    \item sensori \textit{Ultra Wide Band} di presenza (\rif{4.4});
    \item luci (\rif{4.5});
    \item pulsante di allarme (\rif{4.6});
    \item porta di ingresso (\rif{4.7}).
\end{itemize}}
\addInclusione{\rif{4.1}}
\addInclusione{\rif{4.2}}
\addInclusione{\rif{4.3}}
\addInclusione{\rif{4.4}}
\addInclusione{\rif{4.5}}
...
\begin{itemize}
    \item Lo stato corrente del dispositivo (\rif{4.8});
    \item il nome del dispositivo (\rif{4.9});
    \item l'ambiente in cui risiede (\rif{4.10});
    \item le azioni eseguibili sul dispositivo (\rif{4.11}).
\end{itemize}}
\addInclusione{\rif{4.8}}
\addInclusione{\rif{4.9}}
\addInclusione{\rif{4.10}}
\addInclusione{\rif{4.11}}
...
% ======= UC pulsante di allarme =======
\addAttore{\os.}
\addPreCondizione{L'\os si trova nella sezione Dispositivi;}
\addPreCondizione{L'\os ha selezionato un impianto;}
\addPreCondizione{L'\os ha selezionato il dispositivo pulsante di allarme.}
\addPostCondizione{L'\os visualizza il dispositivo pulsante di allarme di un impianto specifico.}
...
\addInclusione{\rif{4.10}}
\addInclusione{\rif{4.11}}
\makeUC{4.6}{Visualizzazione pulsante di allarme}

% ======= UC pulsante di allarme =======
\addAttore{\os.}
\addPreCondizione{L'\os si trova nella sezione Dispositivi;}
\addPreCondizione{L'\os ha selezionato un impianto;}
\addPreCondizione{L'\os ha selezionato il dispositivo porta di ingresso.}
\addPostCondizione{L'\os visualizza il dispositivo porta di ingresso di un impianto specifico.}
...
\addInclusione{\rif{4.10}}
\addInclusione{\rif{4.11}}
\makeUC{4.7}{Visualizzazione porta di ingresso}
...
\makeUC{4.8}{Visualizzazione stato del dispositivo}
...
\makeUC{4.9}{Visualizzazione nome del dispositivo}
...
\makeUC{4.10}{Visualizzazione ambiente del dispositivo}
...
\makeUC{4.11}{Visualizzazione azioni del dispositivo}
...
\addPreCondizione{L'\admin è autenticato nel Sistema;}
\addPreCondizione{L'\admin sta aggiungendo un nuovo allarme;}
\addPreCondizione{Esistono livelli di priorità configurati nel Sistema 4 livelli.}
...
\addPreCondizione{L'\admin è autenticato nel Sistema;}
\addPreCondizione{L'\admin sta aggiungendo un nuovo allarme;}
\addPreCondizione{Esistono gruppi/reparti configurati nel Sistema.}
\addPostCondizione{Il multicast per la notifica dell'allarme è configurato.}
\addTrigger{L'\admin deve configurare a quali gruppi/reparti inviare l'allarme.}
\addScenarioPrincipale{L'\admin visualizza la lista dei gruppi/reparti disponibili;}
\addScenarioPrincipale{L'\admin seleziona i gruppi/reparti che riceveranno la notifica;}
\addScenarioPrincipale{Il Sistema associa il multicast all'allarme.}
\addScenarioAlternativo{Non ci sono gruppi/reparti configurati.}
\addEstensione{Errore configurazione multicast \rif{5.1.4.1}}
...
% ======= UC5.4: Gestione utenti e permessi (admin) =======
\texttt{<diagramma d'uso>}

\addAttore{\admin.}
\addPreCondizione{Il Sistema è attivo;}
\addPreCondizione{L'\admin è autenticato nel Sistema.}
\addPostCondizione{L'\admin ha gestito gli utenti e i permessi del Sistema.}
\addTrigger{L'\admin vuole gestire gli utenti del \os e i relativi permessi.}
\addScenarioPrincipale{L'\admin accede alla sezione di gestione utenti;}
\addScenarioPrincipale{L'\admin registra un nuovo utente del \os;}
\addScenarioPrincipale{L'\admin aggiunge impianti monitorabili all'utente;}
\addScenarioPrincipale{L'\admin rimuove impianti monitorabili dall'utente;}
\addScenarioPrincipale{L'\admin gestisce i permessi degli utenti;}
\addScenarioPrincipale{L'\admin elimina un utente del \os.}
\addScenarioAlternativo{Non ci sono impianti disponibili da assegnare;}
\addScenarioAlternativo{L'operazione di gestione utenti fallisce.}
\addInclusione{Registrazione utente del \os \rif{5.4.1}}
\addInclusione{Aggiungi impianto monitorabile \rif{5.4.2}}
\addInclusione{Rimuovi impianto monitorabile \rif{5.4.3}}
\addInclusione{Eliminazione utente del \os \rif{5.4.4}}
\addInclusione{Gestione permessi utenti \rif{5.4.5}}
\addEstensione{Errore gestione utenti \rif{5.4.6}}
\makeUC{5.4}{Gestione utenti e permessi}

\addAttore{\admin.}
\addPreCondizione{Il Sistema è attivo;}
\addPreCondizione{L'\admin è autenticato nel Sistema;}
\addPreCondizione{L'\admin sta visualizzando la sezione di gestione utenti.}
\addPostCondizione{Il nuovo utente del \os è registrato nel Sistema.}
\addTrigger{L'\admin vuole registrare un nuovo utente del \os.}
\addScenarioPrincipale{L'\admin inserisce i dati anagrafici del nuovo utente del \os (nome, cognome, email);}
\addScenarioPrincipale{L'\admin genera le credenziali di accesso;}
\addScenarioPrincipale{Il Sistema registra il nuovo utente del \os;}
\addScenarioPrincipale{Il Sistema invia le credenziali al nuovo utente via email.}
\addScenarioAlternativo{I dati inseriti sono incompleti o non validi;}
\addScenarioAlternativo{L'email fornita è già registrata nel Sistema;}
\addScenarioAlternativo{Il Sistema non riesce a inviare l'email con le credenziali.}
\addEstensione{Errore registrazione utente del \os \rif{5.4.1.1}}
\makeUC{5.4.1}{Registrazione utente del \os}

\addAttore{\admin.}
\addPreCondizione{Il Sistema è attivo;}
\addPreCondizione{L'\admin è autenticato nel Sistema;}
\addPreCondizione{L'\admin sta tentando di registrare un nuovo utente del \os.}
\addPostCondizione{Il Sistema notifica l'errore e non registra l'utente.}
\addTrigger{I dati inseriti per la registrazione non sono validi.}
\addScenarioPrincipale{Il Sistema verifica i dati inseriti;}
\addScenarioPrincipale{Il Sistema rileva errori (campi mancanti, email duplicata, formato non valido);}
\addScenarioPrincipale{Il Sistema mostra un messaggio di errore con i dettagli specifici.}
\makeUC{5.4.1.1}{Errore registrazione utente del \os}

\addAttore{\admin.}
\addPreCondizione{Il Sistema è attivo;}
\addPreCondizione{L'\admin è autenticato nel Sistema;}
\addPreCondizione{L'\admin sta visualizzando la sezione di gestione utenti;}
\addPreCondizione{Esiste almeno un utente del \os registrato;}
\addPreCondizione{Esistono impianti configurati nel Sistema.}
\addPostCondizione{Gli impianti selezionati sono aggiunti all'utente del \os.}
\addTrigger{L'\admin vuole aggiungere specifici impianti/residenze a un utente del \os.}
\addScenarioPrincipale{L'\admin seleziona un utente del \os;}
\addScenarioPrincipale{L'\admin visualizza la lista degli impianti/residenze disponibili;}
\addScenarioPrincipale{L'\admin seleziona gli impianti da aggiungere;}
\addScenarioPrincipale{Il Sistema aggiunge gli impianti all'utente del \os;}
\addScenarioPrincipale{Il Sistema salva l'assegnazione.}
\addScenarioAlternativo{Non ci sono impianti disponibili da assegnare;}
\addScenarioAlternativo{L'utente del \os selezionato è già assegnato a tutti gli impianti disponibili.}
\addEstensione{Errore aggiunta impianti \rif{5.4.2.1}}
\makeUC{5.4.2}{Aggiungi impianto monitorabile}

\addAttore{\admin.}
\addPreCondizione{Il Sistema è attivo;}
\addPreCondizione{L'\admin è autenticato nel Sistema;}
\addPreCondizione{L'\admin sta tentando di aggiungere impianti a un utente del \os.}
\addPostCondizione{Il Sistema notifica l'errore e non applica l'aggiunta.}
\addTrigger{Si verifica un errore durante l'aggiunta degli impianti.}
\addScenarioPrincipale{Il Sistema tenta di salvare l'aggiunta;}
\addScenarioPrincipale{Il Sistema rileva un errore (es. impianto non più disponibile, problema di connessione);}
\addScenarioPrincipale{Il Sistema mostra un messaggio di errore.}
\makeUC{5.4.2.1}{Errore aggiunta impianti}
...
\addPreCondizione{L'\admin sta visualizzando la sezione di gestione utenti;}
\addPreCondizione{Esiste almeno un utente del \os registrato;}
\addPreCondizione{L'utente del \os ha almeno un impianto assegnato.}
\addPostCondizione{Gli impianti selezionati sono rimossi dall'utente del \os.}
\addTrigger{L'\admin vuole rimuovere specifici impianti/residenze da un utente del \os.}
\addScenarioPrincipale{L'\admin seleziona un utente del \os;}
\addScenarioPrincipale{L'\admin visualizza la lista degli impianti/residenze assegnati;}
\addScenarioPrincipale{L'\admin seleziona gli impianti da rimuovere;}
\addScenarioPrincipale{Il Sistema rimuove gli impianti dall'utente del \os;}
\addScenarioPrincipale{Il Sistema salva la modifica.}
\addScenarioAlternativo{L'utente del \os non ha impianti assegnati;}
\addScenarioAlternativo{L'\admin tenta di rimuovere tutti gli impianti assegnati.}
\addEstensione{Errore rimozione impianti \rif{5.4.3.1}}
\makeUC{5.4.3}{Rimuovi impianto monitorabile}

\addAttore{\admin.}
\addPreCondizione{Il Sistema è attivo;}
\addPreCondizione{L'\admin è autenticato nel Sistema;}
\addPreCondizione{L'\admin sta tentando di rimuovere impianti da un utente del \os.}
\addPostCondizione{Il Sistema notifica l'errore e non applica la rimozione.}
\addTrigger{Si verifica un errore durante la rimozione degli impianti.}
\addScenarioPrincipale{Il Sistema tenta di salvare la rimozione;}
\addScenarioPrincipale{Il Sistema rileva un errore (es. violazione vincoli, problema di connessione);}
\addScenarioPrincipale{Il Sistema mostra un messaggio di errore.}
\makeUC{5.4.3.1}{Errore rimozione impianti}
...
\addPreCondizione{L'\admin sta visualizzando la sezione di gestione utenti;}
\addPreCondizione{Esiste almeno un utente del \os registrato.}
\addPostCondizione{L'utente del \os è eliminato dal Sistema.}
\addTrigger{L'\admin vuole eliminare un utente del \os.}
\addScenarioPrincipale{L'\admin seleziona un utente del \os;}
\addScenarioPrincipale{L'\admin conferma l'eliminazione;}
\addScenarioPrincipale{Il Sistema rimuove l'utente del \os dal Sistema;}
\addScenarioPrincipale{Il Sistema revoca tutti gli accessi e le assegnazioni dell'utente.}
\addScenarioAlternativo{L'\admin annulla l'operazione;}
\addScenarioAlternativo{L'utente ha allarmi attivi in carico.}
\addEstensione{Errore eliminazione utente \rif{5.4.4.1}}
\makeUC{5.4.4}{Eliminazione utente del \os}

\addAttore{\admin.}
\addPreCondizione{Il Sistema è attivo;}
\addPreCondizione{L'\admin è autenticato nel Sistema;}
\addPreCondizione{L'\admin sta tentando di eliminare un utente del \os.}
\addPostCondizione{Il Sistema notifica l'errore e non elimina l'utente.}
\addTrigger{Si verifica un errore durante l'eliminazione dell'utente.}
\addScenarioPrincipale{Il Sistema verifica se l'utente può essere eliminato;}
\addScenarioPrincipale{Il Sistema rileva un errore (es. utente con allarmi attivi, violazione vincoli);}
\addScenarioPrincipale{Il Sistema mostra un messaggio di errore.}
\makeUC{5.4.4.1}{Errore eliminazione utente}
...
\addPreCondizione{L'\admin sta visualizzando la sezione di gestione utenti;}
\addPreCondizione{Esiste almeno un utente del \os registrato.}
\addPostCondizione{I permessi dell'utente del \os sono aggiornati.}
\addTrigger{L'\admin vuole modificare i permessi di un utente del \os.}
\addScenarioPrincipale{L'\admin seleziona un utente del \os;}
\addScenarioPrincipale{L'\admin visualizza i permessi attuali;}
\addScenarioPrincipale{L'\admin modifica i permessi (visualizzazione dashboard, gestione allarmi, accesso analytics, presa in carico allarmi);}
\addScenarioPrincipale{Il Sistema aggiorna i permessi dell'utente;}
\addScenarioPrincipale{Il Sistema salva le modifiche.}
\addScenarioAlternativo{L'\admin tenta di rimuovere tutti i permessi a un utente del \os;}
\addScenarioAlternativo{Si verifica un errore durante il salvataggio dei permessi.}
\addEstensione{Errore gestione permessi \rif{5.4.5.1}}
\makeUC{5.4.5}{Gestione permessi utenti}

\addAttore{\admin.}
\addPreCondizione{Il Sistema è attivo;}
\addPreCondizione{L'\admin è autenticato nel Sistema;}
\addPreCondizione{L'\admin sta tentando di modificare i permessi di un utente del \os.}
\addPostCondizione{Il Sistema notifica l'errore e mantiene i permessi precedenti.}
\addTrigger{La configurazione dei permessi non è valida.}
\addScenarioPrincipale{Il Sistema verifica la configurazione dei permessi;}
\addScenarioPrincipale{Il Sistema rileva che almeno un permesso base deve rimanere attivo;}
\addScenarioPrincipale{Il Sistema mostra un messaggio di errore.}
\makeUC{5.4.5.1}{Errore gestione permessi}

\addAttore{\admin.}
\addPreCondizione{Il Sistema è attivo;}
\addPreCondizione{L'\admin è autenticato nel Sistema;}
\addPreCondizione{L'\admin sta eseguendo un'operazione di gestione utenti.}
\addPostCondizione{Il Sistema notifica l'errore e mantiene lo stato precedente.}
\addTrigger{Si verifica un errore generico durante la gestione utenti.}
\addScenarioPrincipale{Il Sistema tenta di eseguire l'operazione richiesta;}
\addScenarioPrincipale{Il Sistema rileva un errore di sistema (es. database non disponibile);}
\addScenarioPrincipale{Il Sistema mostra un messaggio di errore e suggerisce di riprovare.}
\makeUC{5.4.6}{Errore gestione utenti}
...
% Rimosso, dopo consiglio di Mariano
% Abbiamo la visibilità solo del gateway
