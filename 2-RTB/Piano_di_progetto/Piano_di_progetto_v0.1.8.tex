...
Nel corso del periodo di avanzamento sono state svolte le seguenti attività:
\begin{itemize}
    \item L'Analisi dei Requisiti si è di molto avvicinata alla conclusione, con la redazione degli ultimi \textit{use case} e la crezione dei relativi diagrammi UML. Inoltre è stata modificata secondo le indicazioni forniteci dal prof. Cardin in un ricevimento avvenuto durante questo sprint. Si prevede che venga verificata, validata e conclusa, per la \textit{milestone} RTB, nel corso del prossimo sprint;
    \item L'implementazione del \textit{PoC} ha subito un notevole avanzamento. Anche se questa non è stata completata, sono state implementate e integrate tra loro le tecnologie scelte dal gruppo, e sono state aggiunte funzionalità fondamentali per darne prova; 
    ...
    \item Le attività di questo sprint sono state concentrate principalemente alla preparazione per l'analisi dello stato di avanzamento del progetto con la Proponente, che avverrà il 28/01/2026, alla presenza non solo del referento interno ma anche dei team di \textit{Marketing$_{G}$}, Sviluppo Software, \textit{Cloud$_{G}$} e \textit{Data Analysis$_{G}$}.
\end{itemize}
...
\begin{itemize}
    ...
    \item \rif{RC3} - Disparità di conoscenze tra componenti: alcuni membri del gruppo hanno mostrato minore conoscenza delle tecnologie scelte rispetto ad altri, creando difficoltà nella collaborazione e nel passaggio di consegne tra programmatori.
          \\La mitigazione è avvenuta attraverso sessioni di tutoring e condivisione delle conoscenze tra i membri del gruppo soprattutto tra programmatori.
...
Nonostante i limiti incontrati, il lavoro svolto è stato di notevole importanza: nel \textit{PoC} sono state implementate le entità di Allarme e Soglia come definito nello sprint precedente, rimane da integrare la tecnologia scelta per il \textit{database}, l'Analisi dei Requisiti, dal punto di vista funzionale, è terminata, siamo soddisfatti del numero e del grado di dettaglio dei requisiti trovati. Rimane da completare la parte di modellazione UML e di revisione finale del documento.
