...
\begin{itemize}
    ...
    \item \rif{RO7} - Sovrastima costi e ore: è stata riscontrata una sovrastima nell'assegnazione delle ore per alcuni ruoli (Francesco: -1 ora, Valeria: -7 ore).
          \\La mitigazione è avvenuta attraverso una ripianificazione più accurata per gli sprint successivi.
    \item \rif{RO8} (non previsto) - Sottostima costi e ore: è stata riscontrata una sottostima nell'assegnazione delle ore per alcuni ruoli (Christian: +1 ora, Giuseppe: +4 ore, Leonardo: +4 ore, Filippo: +1 ora).
          \\La mitigazione è avvenuta attraverso una ripianificazione più accurata per gli sprint successivi.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Proof of Concept}: continuazione del lavoro, comprensiva di aggiunta delle entità e delle logiche Allarme e Soglia e della loro visualizzazione;
    \item \textbf{Verbali}: scrittura del verbale interno relativo alla riunione del 29/12/2025 e del verbale esterno relativo al \textit{sal} del 07/01/2026. Correzione e successivo caricamento per ulteriore revisione
          dei documenti revisionati precedentemente su \textit{Round Review}.
    \item \textbf{Presentazioni}: preparazione della presentazione per il Diario di Bordo del 08/01/2026 e per il \textit{SAL} del 07/01/2026.
...
\begin{itemize}
    ...
    \item \rif{R07} - Sovrastima costi e ore: è stata riscontrata una sovrastima nell'assegnazione delle ore per alcuni ruoli (Filippo: -4 ore, Christian: -2 ore).
          \\La mitigazione è avvenuta attraverso una ripianificazione più accurata per gli sprint successivi.
    \item \rif{RO8} (non previsto) - Sottostima costi e ore: è stata riscontrata una sottostima nell'assegnazione delle ore per alcuni ruoli (Francesco: +1 ora, Luca: +1 ora).
          \\La mitigazione è avvenuta attraverso una ripianificazione più accurata per gli sprint successivi.
\end{itemize}
...
\begin{itemize}
    ...
    \item completamento stesura del Piano di Progetto;
    \item completamento stesura del Piano di Qualifica;
...
\begin{itemize}
    ...
    \item Le attività di questo sprint si sono concentrate principalmente sulla preparazione dell'analisi dello stato di avanzamento del progetto con la Proponente, che avverrà il 28/01/2026, in presenza non solo del referente interno ma anche dei \textit{team} di \textit{Marketing$_{G}$}, Sviluppo \textit{Software}, \textit{Cloud$_{G}$} e \textit{Data Analysis$_{G}$}.
\end{itemize}
...
\begin{itemize}
    \item \rif{RI2} - Impegni universitari: il gruppo ha riscontrato difficoltà nel bilanciare gli impegni accademici, derivanti dalla sessione di esami, con le attività di progetto.
          \\La mitigazione è avvenuta attraverso una pianificazione più flessibile delle attività e una comunicazione costante tra i membri del gruppo.
    ...
    \item \rif{RO7} - Sovrastima costi e ore: è stata riscontrata una sovrastima nell'assegnazione delle ore per alcuni ruoli dovuta ai rischi \rif{RI2} e \rif{RO1} (Filippo: -7 ore, Christian: -3 ore, Francesco: -1 ora, Luca: -5 ore).
          \\La mitigazione è avvenuta attraverso una ripianificazione più accurata per gli sprint successivi.
...
Siamo soddisfatti dal numero e dal grado di dettaglio dei requisiti trovati nella relativa Analisi. Rimangono da modellare i diagrammi UML e approvare il documento.  

Complessivamente, in questo sprint non ci si ritiene soddisfatti del lavoro svolto, soprattutto dal punto di vista organizzativo. Le difficoltà incontrate hanno portato a un ritardo significativo rispetto a quanto pianificato, si nota però una capacità sempre crescente di adattamento di fronte a queste. I rischi sono stati per lo più mitigati fin da subito in quanto erano stati quasi tutti previsti.

Si riconosce il fatto che i limiti a cui si è dovuto far fronte servono anche da palestra, per migliorare la gestione del progetto e l'assegnazione delle attività per i prossimi sprint. Dal punto di vista della comunicazione è evidente che questa debba migliorare, non solo tra i programmatori, ma tra tutti i membri del gruppo. Inoltre, la pianificazione delle attività e delle ore assegnate ai ruoli deve essere rivista in modo più accurato, tenendo conto delle difficoltà incontrate in questo sprint.
...
\begin{itemize}
    ...
    \item \textbf{Piano di Qualifica}: aggiornamento del cruscotto di qualità e aggiunta alle tabelle della ripetizione dell'intestazione per migliorare la leggibilità. Il documento è in attesa di approvazione finale;
    \item \textbf{Norme di Progetto}: continuazione della stesura del documento con l'aggiunta di nuove sezioni relative ai processi di supporto. Il documento è in attesa di approvazione finale;
    \item \textbf{Piano di Progetto}: continuazione della stesura con aggiornamento dello stato di avanzamento e sistemazione delle ore individuali dello sprint 7. Il documento è in attesa di approvazione finale;
...
\begin{itemize}
    \item \rif{RI2} - Impegni universitari: durante l'ultimo periodo di avanzamento ci sono stati forti rallentamenti a causa degli esami universitari, che hanno impattato non solo sulla velocità di svolgimento delle attività, ma anche sul processo di approvazione finale dei documenti.
    ...
    \item \rif{RO7} - Sovrastima costi e ore: è stata riscontrata una sovrastima significativa nell'assegnazione delle ore. Rispetto alle 73 ore previste, ne sono state effettivamente svolte 51.5 (-21.5 ore), con particolare eccesso per i ruoli di Verificatore (-11 ore totali: Valeria -4, Francesco -7) e Programmatore (-10 ore totali: Christian -4, Luca -6), mentre si è registrata una sottostima per il ruolo di Amministratore (Filippo +4.5 ore).
...
\setHPrevisteGiuseppe{14}
\setHPrevisteFrancesco{14}
\setHPrevisteLuca{10}
\setHPrevisteLeonardo{8}
...
Nonostante i forti rallentamenti causati dalla sessione di esami invernale, il gruppo ha dimostrato una buona capacità di adattamento, mantenendo il focus sugli obiettivi prioritari. Tuttavia, gli impegni universitari hanno impattato significativamente sul processo di approvazione finale dei documenti: Piano di Progetto, Piano di Qualifica e Norme di Progetto hanno completato la stesura ma sono ancora in attesa della verifica e approvazione finale. Questa difficoltà nel finalizzare le approvazioni ha portato alla decisione di posticipare la candidatura RTB al 13/02/2026, permettendo a tutti i membri del gruppo di dedicare il tempo necessario alle verifiche finali senza compromettere la qualità dei documenti.

Tutti i membri del team si sono espressi soddisfatti del lavoro svolto fin'ora e sono fiduciosi nella qualità dei documenti prodotti, nonostante il ritardo nell'approvazione finale.
