...
da un codice univoco della forma \textit{MPCX}, dove \textit{X} rappresenta 
...
dovrebbe essere completato fino alla fine di un determinato \textit{sprint}. 
...
Questa metrica è utile perché fornisce un punto di riferimento temporale per 
...
\label{sec:MPC2}
\Metrica{MPC2}
{Valore effettivo del lavoro che è stato completato fino alla fine di un determinato \textit{sprint}.
...
Questa metrica è utile per confrontare il lavoro completato sia con i costi 
...
Questa metrica è utile per confrontare le spese effettive 
...
rapporto tra il valore del lavoro completato e il costo reale sostenuto fino ad un 
determinato \textit{sprint}.
Questa metrica è utile per quantificare l'efficienza nell'uso delle risorse e di 
prevedere se il progetto terminerà entro il budget previsto. Un monitoraggio 
...
Questa metrica è utile per valutare se il gruppo ha rispettato le 
...
\label{sec:MPC6}
\Metrica{MPC6}
{Stima del costo totale finale del progetto al completamento, calcolata proiettando 
le performance attuali (sia in termini di costi che di schedulazione) sul lavoro 
rimanente, al termine di un determinato \textit{sprint}. 
La metrica \textit{EAC} fornisce una previsione realistica del budget necessario 
considerando le inefficienze o efficienze riscontrate durante l'esecuzione del progetto. 
La formula si compone di due elementi:
\begin{itemize}
    \item Costi già sostenuti (AC): il totale delle spese effettivamente 
    sostenute fino allo \textit{sprint k};
    \item Stima del lavoro rimanente ($\frac{BAC - EV}{CPI \cdot SPI}$): 
    il valore del lavoro ancora da completare (BAC - EV), corretto in base alle 
    performance attuali di costo (CPI) e schedulazione (SPI).
\end{itemize}
Il denominatore $CPI \cdot SPI$ rappresenta l'efficienza complessiva del progetto: 
valori inferiori a 1 indicano che il lavoro rimanente costerà più del previsto, 
mentre valori superiori a 1 suggeriscono risparmi potenziali. Questa metrica 
consente di identificare tempestivamente potenziali sforamenti di budget, 
permettendo di applicare azioni correttive prima che gli scostamenti 
diventino critici.}
{\[EAC_k = AC_k + \frac{BAC - EV_k}{CPI_k \cdot SPI_k} \]
Dove:
\begin{itemize}
    \item $AC_k$ = \textit{Actual Cost} allo \textit{sprint} k (si veda §\ref{sec:MPC3});
    \item $BAC$ = Budget totale previsto (\textit{Budget At Completion}); 
    \item $EV_k$ = \textit{Earned Value} allo \textit{sprint} k (si veda §\ref{sec:MPC2});
    \item $CPI_k$ = \textit{Cost Performance Index} allo \textit{sprint} k (si veda §\ref{sec:MPC4});
    \item $SPI_k$ = \textit{Schedule Performance Index} allo \textit{sprint} k (si veda §\ref{sec:MPC5});
    \item $k$ = Numero dello sprint di riferimento.
\end{itemize}
}
...
Questa metrica è utile perché permette di valutare se le risorse economiche 
...
Interpretazione dei valori:
\begin{itemize}
    \item RSI $\geq 85\%$: requisiti molto stabili, eccellente analisi iniziale;
    \item $60\% \leq $ RSI $ < 85\%$: stabilità accettabile, alcune modifiche sono normali;
    \item $40\% \leq $ RSI $ < 60\%$: elevata volatilità, problemi nella definizione dei requisiti;
    \item RSI $< 40\%$: instabilità critica.
\end{itemize}}
...
rimanenti e di prevedere la data di completamento del progetto. Una RCV 
stabile indica un processo di sviluppo maturo e prevedibile, mentre variazioni 
significative possono segnalare problemi di stima o complessità variabile dei 
requisiti.}
...
Questa metrica è fondamentale per valutare l'efficacia della pianificazione 
e l'aderenza alle tempistiche: un valore elevato indica problemi sistematici 
nella stima dei tempi, nella gestione delle priorità o nella disponibilità 
...
\trisubsection{MPC11 - Indice di Gulpease (IG)}
\Metrica{MPC11}
{Indice che misura la leggibilità di un testo in lingua italiana, basato sulla 
lunghezza delle parole e delle frasi.
Questa metrica è utile per garantire la qualità della documentazione di 
progetto: assicura che i documenti siano comprensibili, riducendo ambiguità e incomprensioni che potrebbero 
causare errori o ritardi.
Interpretazione dei valori:
\begin{itemize}
...
Questa metrica è fondamentale per garantire la professionalità della 
documentazione.
Interpretazione dei valori:
\begin{itemize}
...
\trisubsection{MPC13 - Indice di Frammentazione (IF)}
\Metrica{MPC13}
...
\trisubsection{MPC14 - Test Success Rate (TSR)}
\Metrica{MPC14}
{Percentuale di casi di test che sono stati eseguiti con esito positivo rispetto al
numero totale di casi di test eseguiti. Questa metrica fornisce una misura della
correttezza del software e dell'efficacia dell'attività di \textit{testing}.
Interpretazione dei valori:
\begin{itemize}
...
\trisubsection{MPC15 - Percentuale Metriche Soddisfatte (PMS)}
\Metrica{MPC15}
...
\trisubsection{MPC16 - Percentuale Rischi Inattesi (PRI)}
\Metrica{MPC16}
{Percentuale di rischi non individuati in fase di analisi iniziale che si sono manifestati
nel corso del progetto. Questa metrica fornisce una misura del livello di accuratezza
dell'attività di analisi e gestione dei rischi.
Interpretazione dei valori:
\begin{itemize}
...
\end{itemize}}
{\[ PRI = \frac{R_{inattesi}}{R_{totali}} \cdot 100 \] 
Dove:
\begin{itemize}
    \item $R_{inattesi}$ = Numero di rischi non individuati che si sono verificati;
    \item $R_{totali}$ = Numero di rischi totali che si sono verificati;
\end{itemize}
}

\trisubsection{MPC17 - Labor Efficiency (LE)}
\Metrica{MPC17}
...
\\Ogni metrica è identificata da un codice univoco della forma \textit{MPDX}, dove \textit{X} rappresenta 
un numero intero progressivo.
...
Questa metrica fornisce una misura delle linee di codice
coperte dai casi di test.
Interpretazione dei valori:
\begin{itemize}
...
\trisubsection{MPD7 - Facilità di Apprendimento (FA)}
\Metrica{MPD7}
{Tempo medio necessario ad un utente per completare un'operazione 
specifica, senza supporto esterno.}
{\[ FA = \frac{\sum_{i=1}^{k} T_i}{k} \]
Dove:
\begin{itemize}
\item $T_{i}$ = Tempo impiegato dall'utente $i$-esimo;
\item $k$ = Numero di utenti testati;
\end{itemize}}
...
\trisubsection{MPD10 - Complessità Ciclomatica (CC)}
\label{sec:MPD10}
\Metrica{MPD10}
