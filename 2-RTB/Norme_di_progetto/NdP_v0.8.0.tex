    ...
    \begin{center}
        ...
        \begin{center}
            \begin{tabular}{|c|c|c|c|}
                ...
                \hline
                0.8.0 & 9/2/2026 & Verificato & SnakeByte, prof. Vardanega Tullio, prof. Cardin Riccardo \\
                \hline
            \end{tabular}
        \end{center}
        \vfill
...
Ogni test è collegato a uno o più requisiti e verifica che il codice li implementi correttamente. Un requisito si intende soddisfatto solo quando tutti i test collegati terminano con esito positivo.\\ 
Durante l’intero ciclo di sviluppo, i test vengono lanciati: mentre si scrivono le funzionalità, dopo ogni modifica o correzione tramite test di regressione, infine nella fase di validazione e collaudo.\\ 
Ripetere i test permette di scoprire subito i difetti introdotti mentre il software evolve e di mantenere il prodotto stabile. \\
...
Ogni test è identificato in modo univoco e ne viene registrato lo stato di avanzamento (non implementato, superato o fallito). L’esito dei test viene documentato e tracciato per supportare le attività di validazione e consentire una valutazione oggettiva della conformità del sistema ai requisiti concordati.
...
dettagliatamente nelle sezioni \textit{§5 Metriche di processo} e \textit{§6 Metriche di prodotto}.
...
\begin{itemize}
    \item \textbf{Microsoft Teams}: piattaforma per incontri online con la Proponente;
    \item \textbf{Verbali esterni}: documenti che tracciano contenuti e decisioni emerse durante i SAL;
    \item \textbf{\textit{Demo}}: presentazioni del prodotto in sviluppo per raccogliere \textit{feedback} operativo.
\end{itemize}

\subsection{Audit}
Il processo di audit consiste in verifiche formali e sistematiche condotte per 
accertare che i prodotti e i processi siano conformi ai requisiti definiti e agli 
standard adottati. Gli audit vengono svolti in corrispondenza delle milestone 
di progetto per garantire la completezza e la qualità degli artefatti prima 
delle consegne ufficiali.

\subsubsection{Audit di baseline}
Prima del raggiungimento di ogni baseline (\textit{Requirements and Technology Baseline}, 
\textit{Product Baseline}), il responsabile conduce un audit formale per verificare:
\begin{itemize}
...
\begin{itemize}
    \item Pianificare e condurre gli audit in corrispondenza delle milestone;
    \item Verificare la conformità degli artefatti rispetto ai criteri stabiliti;
    \item Documentare gli esiti degli audit e le eventuali non conformità rilevate;
    \item Richiedere azioni correttive in caso di difetti o mancanze.
\end{itemize}
Gli audit garantiscono che ogni consegna sia conforme agli standard di progetto 
...
\begin{itemize}
    ...
    \item Milestone di riferimento e priorità;
