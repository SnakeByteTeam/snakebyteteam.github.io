...
\makeMeetingInfoTable{27/11/2025}{16:00}{17:30}{via \textit{Microsoft Teams}}
...
\addPrecedentTodo{vi 2025 11 12.a1}{\noOne}{F. Venzo}{17/11/2025}
\addPrecedentTodo{vi 2025 11 12.a2}{\noOne}{F. Venzo}{17/11/2025}
\addPrecedentTodo{vi 2025 11 12.a3}{\noOne}{C. Libralato}{17/11/2025}
\makePrecedentTodoTable

\section{Ordine del giorno}
\begin{itemize}
    \item Stato avanzamento lavori e discussione del \textit{Gitflow};
    \item valutazione strumenti di \textit{project management};
    \item analisi dei requisiti e proposta di revisione anticipata;
    \item Q\_\&A tecnico-funzionale: autenticazione, permessi, allarmi, gestione impianti, dashboard e interfaccia.
    \item valutazioni su \textit{analytics} e intelligenza artificiale;
    \item debugging autenticazione \textit{KNX IoT 3rd party API};
    \item logistica hardware: consegna del secondo kit di sensori. 
\end{itemize}
...
la definizione delle responsabilità operative e l'aggiornamento delle Norme di Progetto. È stato inoltre creato un template LaTeX per uniformare 
...
Il team è in fase di valutazione tra l'utilizzo di \textit{Jira} e \textit{GitHub Projects}, con il dubbio principale legato al rapporto tra i benefici offerti 
...
Per il processo di revisione, si è discusso l'uso dei commenti nelle Pull Request come strumento di tracciamento 
...
\begin{itemize}
    \item il team ha ricevuto conferma che solo dispositivi specifici (pulsanti, sensori caduta, termostati) generano allarmi, mentre l'attuazione (luci, tapparelle) no;
...
\begin{itemize}
    \item sarebbe auspicabile che la dashboard permetta anche di prendere in carico gli allarmi (azione nice to have).
    ...
    anche se una semplice tabella con filtri è accettata come alternativa se i tempi non lo permettono.
\end{itemize}
...
\subsection*{Logistica hardware: consegna del secondo kit di sensori}
...
\addDecision{Mantenere il flusso Git il più semplice possibile; il team valuterà l'adozione del Feature Branch Flow al posto del complesso Gitflow per il codice sorgente}
...
\addDecision{Il "Gateway Offline" è da considerarsi un allarme ad alta priorità. Le logiche di priorità (es. in base alla frequenza) devono essere parametrizzabili}
\addDecision{Logiche statiche e soglie predefinite per la creazione di \textit{analytics} e suggerimenti}
...
\addTodo{\noOne}{Inviare PDF della presentazione mostrata durante il meeting}{????}{05/12/2025}
\addTodo{\noOne}{Decisione definitiva sul sistema di \textit{project management}}{\teamName}{01/12/2025}
