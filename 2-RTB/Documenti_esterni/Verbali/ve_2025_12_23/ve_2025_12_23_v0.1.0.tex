...
\begin{itemize}
    ...
    \item \textbf{Gestione ore}: dal consuntivo dei primi \textit{sprint$_G$} è emersa una prevedibile differenza tra le ore programmate e quelle reali svolte da ogni ruolo. È dunque sorto il dubbio
    relativo alla gestione delle ore in eccesso, il gruppo ha proposto di mantenere un contatore d'accumulo, da cui attingere in caso
    di sottostime orarie future. In associazione a ciò, il team ha comunicato che spesso è stato necessario effettuare variazioni dei ruoli previsti nello sprint successivo.
    ...
    ha suggerito di effettuare una revisione completa della distribuzione dei ruoli utilizzando eventualmente anche le ore in eccesso. In ogni caso è stato suggerito un confronto
    ...
    per il periodo rimanente dello \textit{sprint} corrente ed eventualmente anche per il periodo successivo, sostituendo la figura del Progettista, che in riferimento all'implementazione del \textit{PoC$_G$}, è stato ritenuto
    meno indispensabile. Anche in questo caso è stato suggerito un confronto con il prof. Vardanega in quanto, sebbene la presenza di più amministratori non sia comune, potrebbe rivelarsi utile per il caso in esame.
\end{itemize}

\subsection*{Stato del \textit{PoC$_G$}}
...
È inoltre stata ricordata l'importanza dello sviluppo di una micro-libreria, come requisito Nice-to-Have, per disaccoppiare la logica applicativa dalle API e fornire una semantica di alto livello, al fine di 
favorire la riutilizzabilità, una maggior strutturazione e un potenziale rilascio open-source.\\\\
...
\addDecision{Struttura ciclo di vita degli allarmi confermata}
\addDecision{Inclusione requisiti non funzionali nell'Analisi dei Requisiti}

\makeDecisionTable

\addTodo{\noOne}{Ottenere un feedback dal prof. Vardanega riguardo la duplicazione Amministratore.}{\teamName}{11/01/2026}
