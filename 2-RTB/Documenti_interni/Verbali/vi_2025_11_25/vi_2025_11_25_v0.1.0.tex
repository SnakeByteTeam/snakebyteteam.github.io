...
Gli analisti hanno presentato la situazione attuale relativamente all'individuazione degli \textit{use case}, sollevando questioni 
...
Prima di proseguire, per allinearsi nell'analisi, ciascuno degli analisti dovrà produrre un \textit{draft$_G$}, anche grafico, 
della possibile \textit{user experience} nell'applicazione, indicando gli \textit{use case} che ne fanno parte. 
...
in modo da poter implementare un sistema di autenticazione amministrativa che sfrutti a pieno l'\textit{API KNX IoT 3rd-party} e una modalità di gestione dei permessi.

\subsection*{Migrazione documentazione e \textit{repository} principale}
Il gruppo ha deciso che la documentazione prodotta finora e contenuta nel \textit{repository Sorgenti} (non so se mettere link visto che verrà dismesso)
...
nell'apposito \textit{template} redatto da L. Pellizzon.  

\subsection*{Assegnazione del \textit{kit} bianco}
Dopo la notifica di \vimarspa della disponibilità di un secondo kit di sensori di colore bianco,
...
\addDecision{Migrazione dei documenti su \teamRepo, dismesso il \textit{repository Sorgenti}}
