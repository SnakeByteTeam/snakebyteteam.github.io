...
\noindent
I test sono suddivisi tra \textbf{backend} e \textbf{frontend} e vengono eseguiti utilizzando gli strumenti forniti rispettivamente da \textit{NestJS}$_G$ e \textit{Angular}$_G$.
...
\begin{itemize}
      \item \textbf{Backend}: Aprire un terminale nella directory \texttt{backend} ed eseguire:
      \begin{verbatim}npm test\end{verbatim}
      
      \textbf{Nota:} questo comando esegue automaticamente anche i test di integrazione.
...
\begin{itemize}
      \item \textbf{Backend}: I test di integrazione sono eseguiti insieme ai test di unità tramite il comando:
      \begin{verbatim}npm test\end{verbatim}
      I test sono organizzati nelle \textit{directory} \texttt{test/unit/} e \texttt{test/integration/}, ma vengono eseguiti congiuntamente dal framework di testing.
      
      \textbf{Nota:} è necessario che il \textit{database} sia in esecuzione.
      \item \textbf{Frontend}: Nel frontend \textit{Angular}, i test di integrazione sono inclusi nei test eseguiti tramite:
      \begin{verbatim}ng test\end{verbatim}
\end{itemize}
...
\begin{itemize}
      ...
      \item  \textbf{Frontend}: Il progetto frontend non include test end-to-end dedicati.
      La copertura dei flussi utente è garantita dai test di accettazione del backend, eseguibili tramite il comando \texttt{npm run test:e2e} nella directory \texttt{backend}.
\end{itemize}
...
\begin{itemize}
    \item   \textbf{Safari}: in versione 26.3
    \item   \textbf{Chrome}: in versione 143
\end{itemize}
...
L'applicazione è comunque progettata per funzionare correttamente anche su macchine di fascia medio-bassa, 
pur beneficiando di risorse aggiuntive in termini di prestazioni e reattività.

A scopo puramente esemplificativo, si riportano di seguito i requisiti minimi consigliati per un ambiente di sviluppo o utilizzo a basso carico:

\begin{itemize}
    \item \textbf{CPU}: processore \textit{multi-core} (almeno 2 \textit{core}) con frequenza di almeno 2.0 GHz, necessario per gestire i \textit{container Docker} e i servizi applicativi;
    \item \textbf{RAM}: almeno 4 GB (8 GB consigliati), considerando l'esecuzione simultanea di molti servizi;
    \item \textbf{Spazio su disco}: almeno 1 GB di spazio libero per immagini \textit{Docker}, \textit{container} e dati persistenti del \textit{database}.
\end{itemize}
...
Il Sistema può essere utilizzato in due modalità: tramite accesso \textit{web} all'istanza già distribuita, oppure tramite installazione locale dalla \textit{repository} su \textit{GitHub}.

\subsection{Utilizzo via web}
\label{sec:utilizzo-web}

La modalità più semplice per accedere al Sistema è tramite browser, collegandosi all'indirizzo dell'istanza distribuita.
Non è richiesta alcuna installazione.

\begin{enumerate}
    \item Aprire il browser supportato (vedere Sezione~\ref{sec:browsers});
    \item Navigare all'indirizzo \href{https://view4life.me}{view4life.me};
    \item Inserire le proprie credenziali nella schermata di accesso e premere \textbf{Accedi} (vedere sezione~\ref{sec:accesso}).
\end{enumerate}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \texttt{CLIENTID}: Identificativo pubblico dell'applicazione;
        \item \texttt{CLIENTSECRET}: Chiave segreta associata al client;
        \item \texttt{HOST1}: Endpoint di autenticazione OpenID Connect;
        \item \texttt{HOST2}: Endpoint per ottenere il token di accesso;
        \item \texttt{REDIRECT\_URI}: URI di redirect dopo autenticazione;
    ...
    \begin{itemize}
        \item \texttt{HOST3}: Endpoint base delle API KNX/Vimar;
        \item \texttt{PLANT\_DOMAIN}: Endpoint di discovery KNX.
    \end{itemize}

    \item \textbf{Credenziali utente MyVimar}
    \begin{itemize}
        \item \texttt{Utente}: Username dell'utente per l'account \texttt{MyVimar};
        \item \texttt{Password}: Password dell'utente per l'account \texttt{MyVimar}.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \texttt{NODE\_SUB\_CALLBACK}: Endpoint callback per le sottoscrizioni;
        \item \texttt{DATAPOINT\_SUB\_CALLBACK}: Endpoint callback per aggiornamenti datapoint.
    \end{itemize}
...
\end{verbatim}
    ...
    \item Aprire il browser e accedere all'indirizzo:
\begin{verbatim}
http://localhost:80
\end{verbatim}
...
\end{verbatim}
    ...
    \item I dati vengono conservati all'interno dei volumi Docker, che rimangono disponibili per successive riaccensioni del Sistema.
\end{enumerate}
...
Tutti gli \textit{endpoint backend} del Sistema sono disponibili alla seguente pagina: \url{https://view4life.me/api/api#}.
