...
\begin{itemize}
  ...
  \item indicazioni su tirocini formativi e opportunità future.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Payload JWT:} Lo \textit{username} fungerà da chiave naturale nel \textit{payload}, mentre la chiave surrogata (ID) resterà confinata nel \textit{database}.
        Il referente della Proponente ha approvato la scelta, raccomandando di documentarla e di privilegiare la tempestiva realizzazione di un \textit{MVP$_G$} funzionante rispetto a ottimizzazioni premature.
\end{itemize}

\subsection{Indicazioni su tirocini formativi e opportunità future}
Il referente della Proponente ha illustrato al gruppo le opportunità di tirocinio:
\begin{itemize}
  \item \textbf{Flessibilità e disponibilità:} Non vi è un limite massimo di posti; la disponibilità varia in base alle necessità dei
        reparti e alle tematiche. Gli studenti possono candidarsi seguendo le proprie tempistiche accademiche.
  \item \textbf{Aree tematiche:} Le tesi potranno riguardare intelligenza artificiale, \textit{IoT}, domotica, accessibilità o
        sviluppo \textit{mobile}, e saranno seguite direttamente dal referente della Proponente o da colleghi specializzati nel settore.
\end{itemize}
