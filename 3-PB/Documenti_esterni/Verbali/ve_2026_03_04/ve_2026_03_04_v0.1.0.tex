...
\newpage 
\addChangelog{0.1.0}{06/03/2026}{L. Granziero}{C. Libralato}{\noOne}{\firstDraft}
\makeChangelog
...
Nel corso dell’incontro sono stati approfonditi gli aspetti relativi alla gestione dell’autenticazione degli utenti e all’integrazione con le API esterne di KNX IOT.
\noindent
È stato chiarito che l’autenticazione degli utenti della Dashboard avverrà tramite credenziali composte da nome utente o email e password salvata in modo sicuro. A seguito del login verrà generato un token JWT, che potrà essere conservato nel local storage o nel session storage del browser e utilizzato per proteggere le chiamate alle API della Dashboard.
\noindent
...
\addDecision{L’integrazione con le API di KNX IOT avverrà tramite protocollo OAuth2 gestito lato server, evitando l’esposizione delle credenziali nel browser degli utenti.}
