...
L'architettura di \textit{deployment} del Sistema è composta da tre \textit{container} \glossario{Docker}: \textit{backend} monolitico, \textit{frontend} come \textit{Single Page Application} sviluppata in Angular, i cui file statici sono serviti, in produzione, da nginx, e \textit{database} PostgreSQL con TimescaleDB. \\ 
L'orchestrazione dei tre \textit{container} avviene tramite l'utilizzo di Docker Compose.\\ \\
I container di \textit{\textbf{backend}} e \textit{\textbf{frontend}} comunicano tra loro tramite API \glossario{RESTful}. \\ \\
