...
\addAtr{+}{api: object}{}
\makeCl{PlantSeekResponseDto}{DTO di risposta per la ricerca di un impianto tramite ID}

% infrastructure\http\dtos\in\subNotification.dto.ts
\addAtr{+}{self: string}{}
\makeCl{NotificationLinkDto}{DTO che rappresenta un link incluso nel payload di una notifica esterna}
...
\makeCl{FetchNewCacheCmd}{Comando che trasporta token e plantId per recuperare la struttura aggiornata di un impianto}

% application\commands\get-valid-cache.command.ts
\addAtr{+}{plantId: string}{}
\makeCl{GetValidCacheCmd}{Comando che trasporta gli ID degli impianti per cui aggiornare la cache}
