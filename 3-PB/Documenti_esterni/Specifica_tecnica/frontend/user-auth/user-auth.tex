...
\makeCl{AuthBaseComponent}{Classe astratta base per i componenti di autenticazione, che gestisce lo stato di caricamento, il tipo di errore e la navigazione post-login tramite segnali reattivi. Definisce il contratto per i metodi di submit e cambio username che le sottoclassi devono implementare.}
...
\makeCl{UserSession}{Interfaccia che rappresenta la sessione di un utente autenticato. Raccoglie le informazioni identificative, il ruolo, il token di accesso e il flag di primo accesso.}

\addAtr{+}{accessToken: string}{}
\makeCl{LoginResponse}{Interfaccia che rappresenta la risposta del backend alla richiesta di login. Espone il token di accesso JWT da utilizzare per le richieste autenticate.}

\addAtr{+}{accessToken: string}{}
\makeCl{RefreshResponse}{Interfaccia che rappresenta la risposta del backend alla richiesta di refresh del token. Espone il nuovo token di accesso JWT.}
...
\makeCl{AuthClaims}{Interfaccia che rappresenta i claims estratti dal payload di un token JWT di autenticazione. Gestisce le varianti di nomenclatura dei campi identificativi e del flag di primo accesso.}

\addAtr{+}{accessToken: string}{}
\makeCl{TokenResponse}{Interfaccia generica che rappresenta una risposta del backend contenente un token di accesso JWT.}
