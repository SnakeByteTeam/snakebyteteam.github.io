    ...
    \begin{center}
        ...
        \begin{center}
            \begin{tabular}{|c|c|c|c|}
                ...
                \hline
                1.1.0                                  & 31/3/2026     & Verificato     & SnakeByte, prof. Vardanega Tullio, prof. Cardin Riccardo \\
                \hline
            \end{tabular}
        \end{center}
        \vfill
...
\begin{xltabular}{\textwidth}{|c|c|c|c|c|X|}
    ...
    \hline
    \endhead
    1.1.0 & 31/3/2026 & L. Granziero & G. De Fina & \makecell[t] - & Aggiunta sezione Scopo del prodotto; aggiunte sottosezioni Strumenti a supporto 
ai processi di Fornitura e Sviluppo; integrate attività mancanti nel processo di Fornitura; aggiunta sezione Codifica con stile, convenzioni e buone pratiche di programmazione; aggiunta sottosezione Strumenti a supporto alla Documentazione; aggiunte sezioni Diari di bordo e Manutenzione dei documenti; aggiunte sezioni Identificazione della configurazione, Controllo della configurazione e Valutazione 
della configurazione; aggiunte sezioni Processo di miglioramento e Processo di formazione.
    \hline
    1.0.0 & 13/2/2026 & - & \makecell[t] - & F. Venzo & Approvazione \\
    \hline
...
\begin{itemize}
    \item un'infrastruttura Cloud containerizzata, progettata secondo il principio di Infrastructure as Code (IaC), che ospita tutte le funzioni dell'applicativo;
    \item un applicativo web responsive destinato al personale sanitario, che include un cruscotto informativo, un sistema di gestione degli allarmi, una sezione analytics con suggerimenti per il risparmio energetico e la visualizzazione dello stato dei dispositivi dell'impianto.
...
Per svolgere le attività previste dal processo di fornitura, il gruppo SnakeByte 
utilizza i seguenti strumenti:
\begin{itemize}
    \item \textbf{Google Calendar}: per segnalare e condividere tra i membri del 
    gruppo gli appuntamenti previsti, quali riunioni interne, incontri con la 
    Proponente e scadenze di progetto;
    \item \textbf{GitHub}: per la gestione delle Issue e del Backlog, utilizzati 
    per tracciare le attività da svolgere. Lo stato di una Issue (aperta o chiusa) 
    riflette direttamente lo stato dell'attività corrispondente;
    \item \textbf{Discord}: per le comunicazioni sincrone interne al gruppo, in 
    particolare per lo svolgimento delle riunioni interne;
    \item \textbf{WhatsApp}: per le comunicazioni asincrone interne al gruppo, 
    quali aggiornamenti rapidi e comunicazioni di servizio;
    \item \textbf{Google Mail}: per le comunicazioni formali scritte verso 
    l'esterno;
    \item \textbf{Microsoft Teams}: utilizzato sia per le comunicazioni asincrone 
    con l'azienda Proponente Vimar S.p.A., tramite la chat dedicata, sia per lo 
    svolgimento delle riunioni esterne in modalità remota.
    \item \textbf{Figma}: utilizzato per la realizzazione dei bozzetti 
    grafici e dei wireframe dell'applicativo web responsive nella fase 
...
\begin{itemize}
    ...
    \item \textbf{Preparazione della risposta}: viene scelto il capitolato per cui ci vuole candidare sulla base delle considerazioni fatte nella fase precedente e viene preparato il documento di \textbf{candidatura}.
     ...
     \item \textbf{Contrattazione}: il gruppo presenta all'azienda Proponente 
    Vimar S.p.A. le risposte elaborate nella fase precedente, con l'obiettivo 
    di concordare i requisiti definitivi e stabilire le modalità di collaborazione;

    \item \textbf{Pianificazione}: definiti i requisiti, il gruppo stabilisce 
    il modello di ciclo di vita da adottare, individua le risorse necessarie 
    allo sviluppo e identifica i potenziali rischi associati, producendo 
    la relativa documentazione di pianificazione;

    \item \textbf{Esecuzione e controllo}: il gruppo realizza quanto pianificato, 
    monitorando costantemente la qualità dei prodotti realizzati e il progresso 
    raggiunto rispetto agli obiettivi prefissati;

    \item \textbf{Revisione e valutazione}: il gruppo mantiene un contatto 
    periodico con Vimar S.p.A. attraverso i SAL (Stato Avanzamento Lavori), 
    durante i quali vengono presentati gli avanzamenti e raccolti feedback 
    sull'andamento del progetto;

    \item \textbf{Consegna e completamento}: ultimato il prodotto, il gruppo 
    provvede alla consegna di quanto realizzato ai committenti e all'azienda 
    Proponente, garantendo il supporto necessario nelle fasi conclusive.
\end{itemize}
...
\subsubsection{Strumenti a supporto}
Per svolgere le attività previste dal processo di Sviluppo, il gruppo SnakeByte 
utilizza i seguenti strumenti:
\begin{itemize}
    \item \textbf{Visual Studio Code}: ambiente di sviluppo integrato utilizzato 
    per la scrittura e la gestione del codice sorgente;
    \item \textbf{Draw.io}: utilizzato per la realizzazione dei diagrammi UML 
    dei casi d'uso individuati in fase di analisi dei requisiti, pubblicati 
    nel relativo documento;
    \item \textbf{StarUML}: utilizzato per la modellazione architetturale e 
    la progettazione del sistema tramite diagrammi UML.
\end{itemize}
...
\trisubsection{Elenco \textit{casi d'uso$_{G}$}}
I {casi d'uso} vengono elencati seguendo la numerazione associata, sono presentati attraverso un diagramma \textit{UML} e una descrizione testuale in forma tabellare contenente sia le informazioni
...
\trisubsection{{Elenco requisiti}}
I requisiti sono identificati da un codice univoco e sono associati ai casi d'uso da cui sono stati generati.

\subsubsection{Codifica}

\trisubsection{Scopo}
La codifica svolta dai programmatori di SnakeByte ha come scopo la realizzazione 
di quanto progettato dagli analisti e dai progettisti. Questa sezione descrive le 
regole e le convenzioni che i programmatori devono seguire per garantire la qualità 
del codice prodotto.

\trisubsection{Stile di codifica: TypeScript}

\trisubsection{Formattazione del codice}
La formattazione del codice TypeScript, sia per il backend (NestJS) che per il 
frontend (Angular), segue lo standard definito da \textbf{Prettier}, eseguito 
automaticamente nell'ambiente locale dello sviluppatore al salvataggio del file. 
La qualità del codice viene inoltre analizzata tramite \textbf{SonarQube}, 
integrato in ambiente di CI/CD tramite \textit{GitHub Actions}, che impedisce 
l'introduzione di codice non conforme agli standard nel branch principale.

\trisubsection{Buone pratiche di programmazione}
\begin{itemize}
    \item \textbf{Variabili globali}: evitare l'uso di variabili globali 
    dove possibile;
    \item \textbf{Funzioni}: preferire funzioni brevi e con una singola 
    responsabilità, evitando funzioni eccessivamente lunghe;
    \item \textbf{Lingua}: utilizzare la lingua italiana per i commenti;
    \item \textbf{Nomi}: utilizzare nomi significativi per variabili, 
    funzioni, metodi e classi;
    \item \textbf{Commenti}: commentare il codice in modo chiaro e conciso; 
    ogni funzione deve essere corredata da un commento che ne descriva 
    lo scopo e i parametri;
    \item \textbf{Indentazione}: utilizzare due spazi per l'indentazione, 
    secondo lo standard di Prettier.
\end{itemize}

\trisubsection{Convenzioni sulle nomenclature e sulla posizione dei file}
Il progetto è strutturato seguendo le convenzioni standard di NestJS per il 
backend e di Angular per il frontend:
\begin{itemize}
    \item \textbf{Nomenclatura dei file}: i file seguono la convenzione 
    \textit{kebab-case} (es. \texttt{user.service.ts}, 
    \texttt{auth.controller.ts});
    \item \textbf{Classi}: i nomi delle classi seguono la convenzione 
    \textit{PascalCase} (es. \texttt{UserService}, \texttt{AuthController});
    \item \textbf{Variabili e funzioni}: i nomi di variabili e funzioni 
    seguono la convenzione \textit{camelCase} (es. \texttt{getUserById}, 
    \texttt{isAuthenticated});
    \item \textbf{Componenti dell'architettura NestJS}: le componenti 
    principali dell'architettura devono avere, nella parte finale del nome, 
    i seguenti suffissi:
    \begin{itemize}
        \item \texttt{Controller}, per la componente responsabile della 
        gestione delle richieste HTTP;
        \item \texttt{Service}, per la componente responsabile della 
        Business Logic;
        \item \texttt{Module}, per la componente responsabile 
        dell'organizzazione e del raggruppamento delle funzionalità;
        \item \texttt{Repository}, per la componente responsabile 
        dell'accesso ai dati su PostgreSQL.
    \end{itemize}
    \item \textbf{Componenti Angular}: i componenti del frontend devono 
    avere i seguenti suffissi:
    \begin{itemize}
        \item \texttt{Component}, per i componenti dell'interfaccia utente;
        \item \texttt{Service}, per i servizi condivisi tra componenti;
        \item \texttt{Module}, per i moduli dell'applicazione;
        \item \texttt{Guard}, per la gestione della protezione delle route.
    \end{itemize}
\end{itemize}
...
\subsubsection{Strumenti a supporto}
Per redigere la documentazione il gruppo SnakeByte utilizza i seguenti strumenti:
\begin{itemize}
    \item \textbf{\LaTeX}: linguaggio di markup utilizzato per la redazione 
    di tutti i documenti di progetto, tramite i template \texttt{template.sty} 
    e \texttt{template} forniti nel repository interno;
    \item \textbf{Hunspell}: strumento per il controllo ortografico e lessicale 
    dei documenti, utilizzato per garantire la correttezza linguistica dei 
    contenuti prodotti;
    \item \textbf{GitHub}: il gruppo utilizza il sistema di pull request di 
    GitHub per la redazione, verifica e approvazione di tutti i documenti. 
    Per ulteriori dettagli consultare la sezione §3.1.4.
\end{itemize}
...
Nell'ambito del progetto di Ingegneria del Software, il prof. Tullio Vardanega 
ha previsto delle attività volte ad accertare, in maniera condivisa, lo stato 
di progresso per ogni singolo gruppo. La struttura di un diario di bordo è 
composta dalle seguenti sezioni:
\begin{itemize}
    \item \textbf{Attività svolte}: elenca quanto è stato realizzato nel 
    periodo compreso tra il diario di bordo precedente e quello corrente;
    \item \textbf{Obiettivi per il periodo successivo}: elenca le attività 
    che il gruppo intende completare nel periodo successivo;
    \item \textbf{Dubbi e difficoltà}: espone le problematiche riscontrate 
    e le questioni ancora aperte nel periodo di riferimento.
\end{itemize}
...
\subsubsection{Manutenzione dei documenti}
L'attività di manutenzione è necessaria nel momento in cui un documento 
esistente necessita di essere aggiornato con nuove informazioni o corretto 
a seguito di segnalazioni. Il processo di aggiornamento non differisce dal 
processo di prima redazione: prevede infatti la creazione di un branch 
dedicato denominato \texttt{modifica-<nome documento>} e il conseguente 
processo di verifica mediante pull request descritto nella sezione §3.1.4.1.
\noindent
\\
Il momento opportuno per sottoporre un aggiornamento a verifica segue 
le stesse regole dell'approvazione, descritte nella sezione §3.1.5: 
le modifiche vengono integrate nel branch \texttt{develop} a seguito 
di verifica positiva e nel branch \texttt{main} solo in corrispondenza 
...
I repository creati per la gestione della configurazione sono i seguenti;
\begin{itemize}
...
Il controllo della configurazione è l'attività che disciplina le richieste 
di modifica, che devono essere notificate e approvate prima di essere 
integrate nel branch principale. Il gruppo SnakeByte attua questa attività 
mediante l'utilizzo delle Issue, della Project Board e delle pull request 
offerte dalla piattaforma GitHub, come descritto di seguito:

\begin{itemize}
    \item \textbf{Issue}: ogni modifica da apportare deve essere documentata 
    mediante una Issue da assegnare al componente che prenderà in carico 
    il lavoro. Ogni Issue è corredata dalle seguenti informazioni:
    \begin{itemize}
        \item un numero univoco che funge da identificativo;
        \item un titolo che descrive sinteticamente l'attività;
        \item una \textit{label} che ne classifica la natura;
        \item un \textit{type} che ne indica la tipologia, scegliendo tra:
        \begin{itemize}
            \item \textbf{Task}: per attività generali di progetto;
            \item \textbf{Feature}: per l'implementazione di nuove 
            funzionalità;
            \item \textbf{Bug}: per la segnalazione e correzione 
            di difetti;
        \end{itemize}
        \item una \textit{milestone} di riferimento che identifica 
        lo sprint o la fase di progetto;
        \item un \textit{project} associato per la gestione 
        nella Project Board.
    \end{itemize}
    Una Issue può essere chiusa esclusivamente quando l'attività 
    corrispondente è stata completata e verificata, portando le modifiche 
    nel branch principale.

    \item \textbf{Project Board}: strumento di GitHub utilizzato per 
    visualizzare lo stato di avanzamento delle attività. Le Issue vengono 
    organizzate nelle seguenti colonne:
    \begin{itemize}
        \item \textbf{Backlog}: attività identificate ma non ancora 
        pianificate;
        \item \textbf{Ready}: attività pianificate per lo sprint corrente;
    ...
    \item \textbf{Pull request}: strumento che permette di richiedere 
    la verifica e l'approvazione delle modifiche prima che queste vengano 
    integrate nel branch principale del repository. Le pull request 
    costituiscono il meccanismo centrale del controllo della configurazione, 
    generando un elenco dettagliato delle modifiche apportate e permettendo 
    al verificatore di richiedere correzioni prima di considerarle definitive. 
    Per ulteriori dettagli sul processo di verifica tramite pull request 
    si rimanda alla sezione §3.1.6.1.
\end{itemize}

\subsubsection{Valutazione della configurazione}
La valutazione della configurazione è l'attività che prevede di controllare 
la completezza del software prodotto rispetto ai suoi requisiti e al design 
scelto. Il gruppo SnakeByte intende realizzare un tracciamento dei requisiti 
per controllare l'aderenza di quanto sviluppato con le aspettative, 
verificando che il prodotto realizzi effettivamente quanto definito 
nell'Analisi dei Requisiti.
\noindent 
\\
Il tracciamento viene realizzato a livello documentale: ogni requisito 
definito nell'Analisi dei Requisiti è associato ai test che ne verificano 
il soddisfacimento, come descritto nel \textit{Piano di Qualifica}. 
Un requisito si intende soddisfatto solo quando tutti i test ad esso 
associati terminano con esito positivo.
\noindent
\\
L'obiettivo è realizzare un prodotto che sia \textit{efficiente}, 
ovvero che soddisfi tutti i requisiti stabiliti, e \textit{sufficiente}, 
nel senso che soddisfi esclusivamente i requisiti necessari senza 
...
apportate al sistema non introducano nuovi difetti o compromettano
...
L'amministratore di sistema (\textit{Sysadmin}) definisce, controlla e mantiene l'ambiente informatico di lavoro.
\begin{itemize}
...
Il processo di miglioramento consiste nello stabilire, consolidare, misurare, 
controllare e migliorare i processi utilizzati durante il ciclo di sviluppo 
del software.

\subsubsection{Attività previste}
Il processo prevede tre attività:
\begin{itemize}
    \item \textbf{Inizializzazione del processo}: stabilire i processi 
    organizzativi e realizzare l'opportuna documentazione, come descritto 
    nel presente documento;
    \item \textbf{Valutazione del processo}: individuare misurazioni 
    appropriate per valutare l'efficienza e l'efficacia dei processi 
    adottati, effettuando controlli periodici sui dati raccolti. 
    Per maggiori informazioni si rimanda alla sezione §5;
    \item \textbf{Miglioramento del processo}: a seguito delle misurazioni 
    effettuate, individuare i processi che presentano criticità e definire 
    opportune azioni correttive. La documentazione viene aggiornata per 
    riflettere i cambiamenti adottati.
\end{itemize}

\subsection{Processo di formazione}
Il processo di formazione ha lo scopo di mantenere i membri di SnakeByte 
aggiornati sui cambiamenti effettuati, nonché definire come gli stessi 
debbano acquisire le conoscenze necessarie allo svolgimento del progetto.

\subsubsection{Attività previste}
Il processo di formazione include le seguenti attività:
\begin{itemize}
    \item \textbf{Implementazione del processo}: realizzazione dei 
    meccanismi necessari alla formazione del gruppo;
    \item \textbf{Sviluppo di materiale per la formazione}: individuazione 
    delle risorse utili all'apprendimento delle tecnologie adottate;
    \item \textbf{Implementazione del piano per la formazione}: definizione 
    di un piano per formare e mantenere formato il gruppo durante 
    l'intero ciclo di vita del progetto.
\end{itemize}

\subsubsection{Implementazione del processo}
Il gruppo SnakeByte ha analizzato i requisiti del progetto e individuato 
le tecnologie da approfondire per portarlo a compimento:
\begin{itemize}
    \item \textbf{NestJS}: framework backend per Node.js;
    \item \textbf{Angular}: framework per lo sviluppo del frontend;
    \item \textbf{PostgreSQL}: sistema di gestione del database relazionale;
    \item \textbf{Docker}: strumento per la containerizzazione 
    dell'infrastruttura;
    \item \textbf{KNX IoT 3rd-party API}: interfaccia REST per la 
    comunicazione con gli impianti Vimar View Wireless;
    \item \textbf{\LaTeX}: linguaggio di markup per la redazione 
    della documentazione.
\end{itemize}

\subsubsection{Risorse per la formazione}
Per l'apprendimento delle tecnologie sopra elencate, il gruppo SnakeByte 
ha adottato le seguenti risorse:

\begin{itemize}
    \item \textbf{NestJS}: documentazione ufficiale disponibile 
    sul sito del framework;
    \item \textbf{Angular}: documentazione ufficiale disponibile 
    sul sito del framework;
    \item \textbf{PostgreSQL}: documentazione ufficiale disponibile 
    sul sito del progetto;
    \item \textbf{Docker}: sessione formativa tenuta dall'azienda 
    Proponente Vimar S.p.A., integrata dalla documentazione 
    ufficiale dello strumento;
    \item \textbf{KNX IoT 3rd-party API}: sessione formativa tenuta 
    dall'azienda Proponente Vimar S.p.A., corredata dalla 
    documentazione tecnica fornita dalla stessa per l'accesso 
    e l'utilizzo dei dispositivi Vimar;
    \item \textbf{\LaTeX}: documentazione ufficiale disponibile 
    sul sito del progetto.
\end{itemize}

\subsubsection{Piano per la formazione}
Il gruppo SnakeByte si impegna a dedicare, durante lo svolgimento 
degli sprint, appositi momenti per la formazione individuale dei 
membri, in particolare nelle fasi iniziali del progetto in cui 
è necessario acquisire familiarità con le tecnologie adottate. 
Qualora lo ritenesse necessario, il gruppo si riserva di pianificare 
sessioni dedicate esclusivamente allo studio, consapevole che 
un forte supporto alla formazione sarà dato dalla realizzazione 
