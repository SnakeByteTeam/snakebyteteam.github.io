    ...
    \begin{center}
        ...
        \begin{center}
            \begin{tabular}{|c|c|c|c|}
                ...
                \hline
                1.0.1                                  & 16/03/2026     & Verificato     & SnakeByte, prof. Vardanega Tullio, prof. Cardin Riccardo \\
                \hline
            \end{tabular}
        \end{center}
        \vfill
...
\begin{xltabular}{\textwidth}{|c|c|c|c|c|X|}
    ...
    \hline
    \endhead
    1.0.1 & 16/03/2026 & F. Venzo & C. Libralato & - & Completamento dei documenti attesi dal processo di fornitura\\
    \hline
...
\begin{itemize}
    ...
    \item \textbf{Contrattazione}: attività che prevede la ricezione del giudizio, riguardante la precedente candidatura, da parte dei committenti$_{G}$ e, in 
    caso di esito positivo un incontro con la Proponente per negoziare i requisiti utente finali;
    
    \item \textbf{Pianificazione}: sulla base dei requisiti stabiliti nell'attività precedente, il fornitore definisce un piano di lavoro che assicuri qualità al prodotto. Inoltre, deve scegliere un modello di ciclo di vita del software appropriato allo scopo, alla grandezza e alla complessità del progetto. In questa attività vengono individuate le risorse necessarie per la realizzazione del prodotto ed i rischi associati al progetto con le relative strategie di mitigazione;
    
    \item \textbf{Esecuzione e controllo}: il fornitre implementa ed esegue il piano di gestione definito nell'attività precedente, monitorando il progresso, i costi e la qualità dei processo e dei prodotti, apportando eventuali modifiche al piano di gestione e/o di qualità in risposta ai problemi i quali devono essere identificati, registrati, analizzati e risolti.
    
    \item \textbf{Revisione e valutazione}: il fornitore deve comunicare constantemente con la Proponente, al fine di revisionare e valutare lo stato del progetto e di dimostrare la conformità dei prodotti ai requisiti dell'acquirente.
    
    \item \textbf{Consegna e completamento}: il fornitore consegna il prodotto come specificato dal contratto stipulato con i committenti, garantedo il supporto adeguato.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Piano di Progetto} contenente
          \begin{itemize}
              \item Pianificazione delle risorse divisa per periodo (\textit{sprint$_{G}$});
              \item Analisi dei rischi e strategie di mitigazione adottate;
              \item Retrospettiva sulle attività di ogni sprint ed individuazione di eventuali azioni correttive da adottare nei successivi \textit{sprint}.
          \end{itemize}

    \item \textbf{Piano di Qualifica} contenente 
          \begin{itemize}
            \item Piano della Qualità del progetto, attività mirate a fissare gli obiettivi di qualità, insieme con i processi e le risorse necessarie per conseguirli;
            \item Controllo di Qualità, le attività pianificate e attuate per assicurare che il prodotto soddisfi le attese;
            \item Cruscotto di Valutazione, che riporta misurazioni e valutazioni, ragionando sulle tendenze passate e attese; 
            \item Miglioramento continuo, ovvero individuazione di specifiche modifiche alle attività che ne possono aumentare l'efficacia e l'efficienza.
          \end{itemize}

    \item \textbf{Norme di progetto} (l'attuale documento) contenente 
          \begin{itemize}
            \item Il \textit{Way of Working$_{G}$} adottato dal gruppo, ovvero l'insieme di norme, regole e procedure che guidano l'attuazione delle attività dei processi di ciclo di vita.
          \end{itemize}

    \item \textbf{Glossario} contenente 
          \begin{itemize}
            \item Le definizioni dei termini tecnici e delle espressioni che possono risultare sconosciute o ambigue;
            \item L'interpretazione che il gruppo dà ad alcuni termini comuni, al fine di escludere fraintedimenti e garantire una comunicazione efficace.
          \end{itemize}

    \item \textbf{Verbale esterno} contenente
          \begin{itemize}
            \item Le informazioni generali di ogni incontro con la Proponente cui data, orario, modalità di svolgimento, presenze e ruoli attuali di tutti i membri del gruppo \textit{SnakeByte};
            \item Riassunto degli argomenti trattati durante l'incontro, con approfondimenti necessari;
            \item Tabella delle decisioni prese durante l'incontro, con identificativo e descrizione di ogni decisione;
            \item Attività da svolgere emerse durante l'incontro, con identificativo, descrizione, assegnatario e scadenza di completamento;
          \end{itemize}
        Ogni verbali esterno è sottoposto anche alla verifica della Proponente, tramite l'applicativo \textit{ad-hoc} \textit{RoundReview} la quale può verificarlo apponendo la propria firma, oppure richiedere modifiche al documento.

    \item \textbf{Verbale interno} contenente
          \begin{itemize}
            \item Le informazioni generali di ogni incontro tra i componenti del gruppo cui data, orario, modalità di svolgimento, presenze e ruoli attuali di ogni membro;
            \item Le attività emerse da precedenti incontri e completate, con relativo identificativo, identificativo della \textit{GitHub$_{G}$ Issue$_{G}$} associata, assegnatario e data di completamento;
            \item Riassunto degli argomenti trattati durante l'incontro, con approfondimenti necessari;
            \item Tabella delle decisioni prese durante l'incontro, con identificativo e descrizione di ogni decisione;
            \item Attività da svolgere emerse durante l'incontro, con identificativo, identificativo della \textit{GitHub Issue} associata, descrizione, assegnatario e scadenza di completamento;
          \end{itemize}
\end{itemize}

\subsubsection{Strumenti}
Per facilitare l'attuazione delle attività del processo di fornitura, il gruppo ha deciso di adottare i seguenti strumenti: 
\begin{itemize}
    \item \textit{Google Sheets$_{G}$} per la gestione dei ruoli, degli orari previsiti ed effettuati di ogni ruolo e di ogni membro, e per il calcolo delle metriche del Cruscotto di Qualità;
\end{itemize}
