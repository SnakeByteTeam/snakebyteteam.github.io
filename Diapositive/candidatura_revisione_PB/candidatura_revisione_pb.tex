...
\begin{itemize}
  \item Frontend: \textit{Sigle Page Application} in Angular, servita da \textit{nginx};
  \item Backend: monolite.
\end{itemize}
\vspace{0.2cm}
Entrambi, come il database \textit{PostegreSQL}, sono containerizzati e orchestrati da \textit{Docker Compose}. 
\vspace{0.2cm}
...
\begin{itemize}
  ...
  \item Soluzione: \textbf{Architettura MVVM} che separa le tre responsabilità:
        \begin{itemize}
          \item \textbf{Model}: i dati e la logica di dominio;
          \item \textbf{View}: il template di Angular;
          \item \textbf{ViewModel}: un servizio che espone stato e comandi alla View tramite \textbf{Observable}.
        \end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
  ...
  \item Soluzione: pattern \textbf{Dependency Injection}
          \begin{itemize}
            ...
            \item In \textit{TypeScript (Nest JS)} il pattern è supportato nativamente tramite 
...
\begin{itemize}
  ...
  \begin{itemize}
    \item Permette di cambiare a \textit{runtime} l'algoritmo per il calcolo delle \textit{analytics};
    \item È stata realizza un'interfaccia \texttt{AnalyticsStrategy} che fornisce il metodo \texttt{execute}, che le \textit{strategies} implementano.
  \end{itemize}
\end{itemize}
\end{frame}
