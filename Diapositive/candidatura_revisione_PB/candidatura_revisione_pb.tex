...
\begin{frame}{Comunicazione tra \textit{frontend} e \textit{backend}}
La comunicazione tra \textit{frontend} e \textit{backend} avviene tramite \textit{API REST}.
\begin{figure}
\centering
\begin{minipage}{0.5\textwidth}
  \centering
  \includegraphics[width=0.95\linewidth]{./img/alarmrulesswagger.png}
  \captionof{figure}{\textit{route} per le regole di allarme}
  \label{fig:test1}
\end{minipage}%
\begin{minipage}{0.5\textwidth}
  \centering
  \includegraphics[width=0.95\linewidth]{./img/alarmeventswagger.png}
  \captionof{figure}{\textit{route} per gli eventi di allarme}
  \label{fig:test2}
\end{minipage}
\end{figure}
\end{frame}

\begin{frame}{Architettura logica \textit{backend}}
\begin{itemize}
  \item Problema:
  \begin{itemize}
    \item La logica di \textit{business} del \textit{backend} dipende dai dettagli infrastrutturali (\textit{Persistent Logic})
    e dalla tecnologia scelta per la codifica;
  ...
  \item Soluzione: \textbf{Architettura Esagonale}
        \begin{itemize}
          \item La logica di \textit{business} è isolata dalle altre componenti;
          \item Tramite le \textbf{Ports} e \textbf{Adapters}, situati ai confini della logica di \textit{business}, è possibile 
          interfacciarsi con essa;
          \item La \textit{business} logic può essere verificata in modo indipendente.
        \end{itemize}
\end{itemize}
\end{frame}
...
\begin{center}
   \includegraphics[width=0.925\textwidth]{img/AlarmUML.png}
\end{center}
\end{frame}
...
\begin{frame}{Architettura logica \textit{frontend}}
\begin{itemize}
  \item Problema:
  \begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{ViewModel}: la classe del componente (\texttt{@Component});
        fa da mediatore tra Model e View, esponendo i dati tramite
        \texttt{Observable} senza contenere logica di dominio.
\end{itemize}
\end{itemize}
\end{frame}
...
\begin{center}
  \includegraphics[width=0.95\textwidth]{img/notification.pdf}
\end{center}
\end{frame}
...
\begin{frame}{Code snippets Notification (Frontend)}
  \begin{figure}
    ...
    \caption{notification-page-component.html}
  \end{figure}
...
\begin{frame}{Design Pattern - Dependency Injection (1/2)}
\begin{itemize}
  \item Problema:
        \begin{itemize}
  ...
  \item Soluzione: pattern \textbf{Dependency Injection}
          \begin{itemize}
            \item Le dipendenze vengono fornite tramite un \texttt{INJECTOR}, che si occupa di creare e gestire il ciclo di vita degli oggetti al posto delle classi stesse;
            \item Il pattern è supportato nativamente in entrambi i \textit{framework} tramite il decoratore \texttt{@Injectable()}: in \textit{NestJS} il \textit{binding} è 
            esplicito tramite il sistema \texttt{providers}, mentre in \textit{Angular} i service si registrano 
            automaticamente nel container tramite \texttt{providedIn:'root'}.
                \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Design Pattern - Dependency Injection (2/2)}
\begin{figure}
  \centering
    \includegraphics[width=0.7\textwidth]{img/providers.png}
    \caption{Sistema \textit{providers} di \textit{NestJS} per la D.I.}
\end{figure}
...
\begin{minipage}{0.4\textwidth}
  \centering
  \includegraphics[width=0.75\textwidth]{img/injectable.png}
  \caption{Sistema \textit{@Injectable} di \textit{NestJS} per la D.I.}
  \label{fig:test1}
\end{minipage}%
\begin{minipage}{0.6\textwidth}
  \centering
  \includegraphics[width=0.925\textwidth]{img/inject.png}
  \caption{Sistema \textit{@Inject} di \textit{NestJS} per la D.I.}
  \label{fig:test2}
\end{minipage}
\end{figure}
...
\begin{frame}{Design Pattern - Observer (1/3)}
\begin{itemize}
    \item Problema:
    \begin{itemize}
      \item I componenti \textit{Angular} devono reagire a eventi asincroni 
      e aggiornare lo stato \textit{UI}.
    \end{itemize}
    \item Soluzione: design pattern \textbf{Observer}
    \begin{itemize}
      \item Implementato tramite libreria \textit{RxJS}: le entità si iscrivono 
      agli \textit{Observable} senza conoscere la sorgente dei dati;
      \item Gli \textit{Observable} gestiscono eventi in maniera automatica ed efficiente mantenendo disaccoppiamento tra le parti coinvolte.
    \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}{Design Pattern - Observer (2/3)}
\begin{center}
    \includegraphics[width=1\textwidth]{img/analytics.pdf}
\end{center}
\end{frame}

\begin{frame}{Design Pattern - Observer (3/3)}
\begin{figure}
  \centering
    \includegraphics[width=0.8\textwidth]{img/analyticsSnippet.png}
    \caption{analytics.component.ts}
    \includegraphics[width=0.6\textwidth]{img/analyticsSnippetTemplate.png}
    \caption{Iscrizione all'interno del template tramite \texttt{async pipe}}
\end{figure}
\end{frame}

\begin{frame}{Design Pattern - Strategy (1/2)}
\begin{itemize}
  \item Problema: 
  \begin{itemize}
...
\begin{frame}{Design Pattern - Strategy (2/2)}
  \begin{center}
