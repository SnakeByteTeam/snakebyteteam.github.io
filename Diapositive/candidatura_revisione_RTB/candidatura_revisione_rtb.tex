...
\setbeamertemplate{footline}{
  ...
  \hbox{
    \begin{minipage}[t]{0.40\paperwidth}
      \hspace*{0.5em}
      \raggedright\usebeamerfont{footline}{Presentazione tecnologie 18/02/2026}
    \end{minipage}%
...
%   \centering
%   \includegraphics[width=0.385\textwidth]{img/tslogo.png}\\
%   TypeScript
...
\begin{frame}{Tecnologie front-end}
\begin{columns}[c] % [c] = centratura verticale
  \column{0.33\textwidth}

  \begin{center}
    \includegraphics[width=0.375\textwidth]{img/tslogo.png}\\
    TypeScript\\
    \scriptsize (\textit{Tipizzazione forte, \\ interfacce, modificatori \\ di accesso})
  ...
  \begin{center}
    \includegraphics[width=0.385\textwidth]{img/angularlogo.png}\\
    Angular\\
    \scriptsize (\textit{Componibilità, completezza, \\test integrati})
  ...
  \begin{center}
    ...
    Docker\\ %alternative Podman (è daemonless e rootless di default ma anche docker lo si può far girare rootless) docker è più diffuso 
            %e meglio supportato (Dockerhub con tutte le versioni ecc.)
    \scriptsize (\textit{Portabilità, \\riproducibilità})
...
\begin{frame}{Tecnologie back-end}
\begin{columns}[c] % [c] = centratura verticale
  \column{0.33\textwidth}
  \centering
  \includegraphics[width=0.375\textwidth]{img/tslogo.png}\\
  ...
  \includegraphics[width=0.7\textwidth]{img/nodejslogo.png}\\
  Node.js \\
  {\scriptsize(\textit{Gestione delle configurazioni/versioni})\par}
  ...
  \scriptsize (\textit{Flessibilità, documentazione estesa})

  \column{0.33\textwidth}
  \centering
  \includegraphics[width=0.385\textwidth]{img/postgrelogo.png}\\
  PostgreSQL\\
  \scriptsize (\textit{Velocità in lettura, \\versatile}) 
  %Scritture molto più lente delle letture 
  % Versatile nel senso che tramite le tabelle UNLOGGED (se saltano il WA Log), JSONB + GIN Index può essere usato come database per caching (tipo redis o mongodb)

  \vspace{0.6cm}

  \centering
  \includegraphics[width=0.75\textwidth]{img/dockerlogo.png}\\
  \normalsize Docker %alternative Podman (è daemonless e rootless di default ma anche docker lo si può far girare rootless) docker è più diffuso 
          %e meglio supportato (Dockerhub con tutte le versioni ecc.)
\end{columns}
\end{frame}


\begin{frame}{Funzionalità implementate nel PoC}
\begin{itemize}
  \item Collegamento account \textit{MyVimar};
  \item Visualizzazione elenco dispositivi (termostati, luci, tapparelle);
  \item Aggiunta allarme (sensore, priorità e soglia di scatto);
